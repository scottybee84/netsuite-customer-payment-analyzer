...
define(["require", "exports", "N/ui/dialog"], function (require, exports, dialog) {
    ...
    exports.analyzeCustomerPerformance = analyzeCustomerPerformance;
    exports.showTopRiskCustomers = showTopRiskCustomers;
    exports.showCashFlowForecast = showCashFlowForecast;
    ...
    function resetButton(button, originalText, className) {
        if (!button)
            return;
    ...
    function renderPaymentAnalytics(data, resultDiv) {
        if (!resultDiv)
            resultDiv = document.getElementById("analytics-result");
        if (!resultDiv)
            return;
        ...
        ${riskAssessment.factors.length > 0 ? `
          <div style="font-size: 11px;">
            ...
            <ul style="margin: 8px 0; padding-left: 20px;">
              ${riskAssessment.factors.map((factor) => `<li style="margin: 3px 0;">${factor}</li>`).join('')}
            </ul>
          </div>
        ` : ''}
      </div>
    ...
    function renderRiskCustomers(customers, resultDiv) {
        if (!resultDiv)
            resultDiv = document.getElementById("analytics-result");
        if (!resultDiv)
            return;
    ...
    function renderCashFlowForecast(forecast, resultDiv) {
        if (!resultDiv)
            resultDiv = document.getElementById("analytics-result");
        if (!resultDiv)
            return;
    ...
    function renderError(message, resultDiv) {
        if (!resultDiv)
            resultDiv = document.getElementById("analytics-result");
        if (!resultDiv)
            return;
    ...
    function analyzeCustomerPerformance() {
