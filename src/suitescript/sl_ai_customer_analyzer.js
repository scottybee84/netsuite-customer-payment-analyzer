 ...
 */
define(["require", "exports", "N/ui/serverWidget", "N/https", "N/search", "N/log", "N/runtime"], function (require, exports, serverWidget, https, search, log, runtime) {
    "use strict";
    ...
    function onRequest(ctx) {
        if (ctx.request && ctx.request.method === "POST" && ctx.request.body) {
            ...
            try {
                ...
                ctx.response.write(JSON.stringify(createErrorResponse("Invalid JSON in request body")));
                return;
            }
            const customerId = body && body.customerId;
            if (!customerId) {
                ctx.response.write(JSON.stringify(createErrorResponse("Customer ID is required")));
                return;
            }
            try {
                log.debug({
                ...
                ctx.response.write(JSON.stringify(createSuccessResponse(searchRes, aiText)));
            }
            catch (err) {
    ...
    function invoiceRowHandler(row) {
        if (!_currentInvoices)
            return false;
        _currentRecordCount++;
    ...
    function runInvoiceSearch(customerId, cap) {
        cap = cap || 20;
        const invoices = [];
    ...
    function callGeminiForAnalysis(invoices) {
        const payload = {
            contents: [
                {
                    parts: [
                        {
                            text: "Analyze AR risk and payment behavior for customer with " +
                                invoices.length +
                                " invoices: " +
                                JSON.stringify(invoices) +
        ...
        if (!resp || resp.code >= 400)
            throw new Error("AI call failed: " + (resp && resp.code) + " " + (resp && resp.body));
    ...
    function createSuccessResponse(searchRes, aiText) {
        return {
            success: true,
            invoiceCount: (searchRes && searchRes.count) ||
    ...
    function getGeminiApiKey() {
        ...
        try {
            if (typeof globalThis !== "undefined" &&
                ...
                return globalThis.GEMINI_API_KEY;
            }
        }
        catch (e) { }
        try {
            const s = runtime.getCurrentScript && runtime.getCurrentScript();
            if (s && typeof s.getParameter === "function") {
                return (s.getParameter("custscript_gemini_api_key") ||
                    s.getParameter({ name: "custscript_gemini_api_key" }) ||
                    "");
            }
        }
        catch (e) { }
