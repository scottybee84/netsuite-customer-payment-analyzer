 ...
 */
define(["require", "exports", "N/ui/serverWidget", "N/search", "N/log"], function (require, exports, serverWidget, search, log) {
    "use strict";
    ...
    function onRequest(ctx) {
        if (ctx.request && ctx.request.method === "POST" && ctx.request.body) {
            ...
            ctx.response.write(JSON.stringify(createErrorResponse("Invalid request parameters")));
            return;
        }
        const form = buildForm();
        ctx.response.writePage(form);
    }
    function getPaymentAnalytics(customerId) {
        try {
    ...
    function calculatePaymentScore(invoices) {
        ...
        if (totalOutstanding > 50000)
            score -= 20;
        else if (totalOutstanding > 25000)
            score -= 15;
        else if (totalOutstanding > 10000)
            score -= 10;
    ...
    function calculateRiskFactors(customer, invoices) {
        const risk = {
            score: 0,
            level: "Low",
            factors: [],
    ...
    function generateRecommendations(paymentScore, riskAssessment) {
    ...
    function getTopRiskCustomers() {
        try {
            ...
            if (rs && typeof rs.each === "function") {
                rs.each((result) => {
                    const balance = parseFloat(result.getValue("balance")) || 0;
                    const creditlimit = parseFloat(result.getValue("creditlimit")) || 0;
                    // Calculate risk score
                    let riskScore = 0;
                    if (balance > creditlimit * 0.9)
                        riskScore += 40;
                    else if (balance > creditlimit * 0.7)
                        riskScore += 25;
                    if (balance > 25000)
                        riskScore += 30;
    ...
    function getCashFlowForecast() {
        try {
            ...
            if (rs && typeof rs.each === "function") {
                rs.each((result) => {
                    const total = parseFloat(result.getValue("total")) || 0;
